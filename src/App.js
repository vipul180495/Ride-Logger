...
const BACKEND_URL = "https://ride-logger-backend-2.onrender.com"; // replace with deployed backend URL
...
const FaceLockLogger = () => {
  ...
  const [formData, setFormData] = useState({
    Driver: "", Annotator: "", Date: "", Vehicle: "", RSUNo: "", RSUStartDate: "", DriveId: ""
  });
  ...
  const verifyFace = async () => {
    ...
    try {
      ...
      if (data.success) {
        setLocked(false);
        if (videoRef.srcObject) videoRef.srcObject.getTracks().forEach(track => track.stop());
      } else alert("Face not recognized or not approved yet.");
    } catch (err) {
      alert("Face verification failed");
    }
  };

  // --- Session time and alerts
  const sessionMinutes = sessionStart ? Math.floor((Date.now() - sessionStart) / 60000) : 0;
  useEffect(() => {
    if (!sessionStart) return;
    if (sessionMinutes === 30 && !notified30) { setBannerMessage("⏰ 30 minutes reached!"); setBannerColor("yellow"); setNotified30(true); }
    if (sessionMinutes === 40 && !notified40) { setBannerMessage("⏰ 40 minutes reached!"); setBannerColor("red"); setNotified40(true); setTimeout(() => { setBannerMessage(""); setBannerColor(""); }, 5000); }
  }, [sessionMinutes, sessionStart, notified30, notified40]);

  // --- Ride logger handlers
    ...
    setTimers(prev => {
      ...
      categories[category].forEach(cond => {
        ...
        if (k !== key && updated[k]) {
          ...
          delete updated[k]; stoppedKey = k;
        }
      });
      if (updated[key]) {
        const duration = Date.now() - updated[key];
        setLogs(l => ({ ...l, [key]: (l[key] || 0) + duration }));
        delete updated[key]; stoppedKey = key;
      } else updated[key] = Date.now();
      if (stoppedKey) { setRecentStopped(stoppedKey); setTimeout(() => setRecentStopped(""), 2000); }
      return updated;
    });
  };
  ...
  const resetCategory = (category) => {
    ...
    categories[category].forEach(cond => { const key = `${category}-${cond}`; delete updatedLogs[key]; delete updatedTimers[key]; });
    setLogs(updatedLogs); setTimers(updatedTimers);
  };

  const stopAll = () => {
    const updatedLogs = { ...logs };
    Object.keys(timers).forEach(key => { const duration = Date.now() - timers[key]; updatedLogs[key] = (updatedLogs[key] || 0) + duration; });
    setLogs(updatedLogs); setTimers({});
  };
  ...
  const exportCSV = () => {
    ...
    csv += "Category,Condition,HH:MM:SS\n";
  ...
  const getTotalMs = key => { const base = logs[key] || 0; const running = timers[key] ? Date.now() - timers[key] : 0; return base + running; };
  ...
  if (locked)
    return (
      <div style={{ textAlign: "center", padding: "30px" }}>
        ...
        <video autoPlay muted ref={ref => { if(ref && !videoRef) { setVideoRef(ref); startCamera(ref); } }} width="350" style={{ borderRadius:"10px", marginBottom:"10px" }} />
        <div>
          <button onClick={registerFace} style={{ padding:"12px 25px", fontSize:"16px", marginRight:"10px" }}>Register Face</button>
          <button onClick={verifyFace} style={{ padding:"12px 25px", fontSize:"16px" }}>Unlock</button>
        </div>
      </div>
    );
    ...
    <div style={{ padding:"25px", fontFamily:"Segoe UI, sans-serif", backgroundColor:"#0d0d0d", color:"#f5f5f5", minHeight:"100vh" }}>
      {bannerMessage && (
        <div style={{
          ...
          color:"black", textAlign:"center", fontSize:"18px", fontWeight:"bold", zIndex:9999
        }}>{bannerMessage}</div>
      )}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(220px,1fr))", gap:"10px", backgroundColor:"#1a1a1a", padding:"15px", borderRadius:"10px", marginBottom:"20px", boxShadow:"0 0 10px #111" }}>
        {Object.keys(formData).map(key => (
          <input key={key} name={key} placeholder={key.replace(/([A-Z])/g," $1")} value={formData[key]} onChange={handleInputChange}
      ...
      <table style={{ width:"100%", borderCollapse:"collapse", backgroundColor:"#1a1a1a", borderRadius:"10px", overflow:"hidden" }}>
        ...
        <tbody>
          {Object.keys(categories).map(category => categories[category].map((condition,index)=>{
            const key = `${category}-${condition}`;
            ...
            return (
              <tr key={key} style={{ backgroundColor:isRecent?"#664400":active?"#332222":"#111", borderBottom:"1px solid #333" }}>
                ...
                <td style={{ padding:"8px" }}>
                  <button onClick={()=>handleConditionClick(category,condition)}
                    style={{
                      padding:"8px 12px", fontSize:"14px", backgroundColor: active?"#e74c3c":"#27ae60",
                ...
                <td style={{ textAlign:"center", padding:"8px", color: active?"#00ff99":isRecent?"#aaa":"" }}>{active?"Running":isRecent?"Stopped":""}</td>
              </tr>
            )
          }))}
        </tbody>
      ...
      <div style={{ marginTop:"20px", textAlign:"center" }}>
        <textarea placeholder="📝 Enter comments here..." value={comment} onChange={e=>setComment(e.target.value)} rows="3"
      ...
      <div style={{ marginTop:"25px", display:"flex", flexWrap:"wrap", justifyContent:"center", gap:"10px" }}>
        {Object.keys(categories).map(cat=>(
          <button key={cat} onClick={()=>resetCategory(cat)} style={{ padding:"10px 15px", backgroundColor:"#ff9900", color:"#fff", border:"none", borderRadius:"8px", cursor:"pointer" }}>
            Reset {cat}
          </button>
        ))}

        <button onClick={stopAll} style={{ padding:"15px 25px", backgroundColor:"#c0392b", color:"white", border:"none", borderRadius:"10px", cursor:"pointer" }}>⏹ Stop All</button>
        <button onClick={exportCSV} style={{ padding:"15px 25px", backgroundColor:"#2980b9", color:"white", border:"none", borderRadius:"10px", cursor:"pointer" }}>📁 Export CSV</button>
      </div>
    </div>
  );
};
